...
│   │   └── loader.go
│   │
│   └── agentdef/                # YAML agent definitions (instructions, tools bundles)
│       └── agentdef.go
│
├── agents/                      # OPTIONAL: checked-in example agent YAML for CLI -agent
│
...
| `agents/` | Example and user agent YAML files consumed by `cmd/centrai -agent` (not Go code). |
